...
SELECT models.name AS model_name, brands.name AS brand_name, brands.headquarters AS headquarters FROM models JOIN brands ON models.brand_id = brands.brand_id WHERE models.name = 'Mustang';
...
SELECT COUNT(*) FROM brands JOIN models ON brands.brand_id = models.brand_id WHERE brands.name = 'Ford';
...
SELECT * FROM models LIMIT 10 OFFSET 14;
...
SELECT b.name, m.name, m.year, b.discontinued, (b.discontinued - m.year) AS years_until_discontinued FROM models AS m LEFT JOIN brands AS b ON m.brand_id = b.brand_id WHERE b.discontinued IS NOT NULL ORDER BY b.name, m.name, m.year;
